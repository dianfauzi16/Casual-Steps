...
if (isset($_GET['id_order']) && filter_var($_GET['id_order'], FILTER_VALIDATE_INT)) {
    ...
    $sql_order = "SELECT id, nama_pelanggan, email_pelanggan, telepon_pelanggan, 
                  ...
                  FROM orders 
                  WHERE id = ? AND user_id = ?"; // Tambahkan pengecekan user_id
    ...
    if ($order_details && empty($error_message)) {
        ...
        if ($stmt_items = $conn->prepare($sql_items)) {
            ...
            if (empty($order_items)) {
                 $error_message = "Tidak ada item ditemukan untuk pesanan ini."; // Seharusnya tidak terjadi jika order ada
            }
        } else {
                ...
                <div class="d-flex align-items-center"> 
                    ...
                    <a href="keranjang.php" class="position-relative me-3 text-dark"> 
                        ...
                        if (isset($_SESSION['keranjang']) && is_array($_SESSION['keranjang'])) {
                            foreach ($_SESSION['keranjang'] as $kuantitas_item) {
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-striped table-hover mb-0 item-table">
                                    ...
                                    <tfoot>
                                        <tr class="fw-bold">
                                            <td colspan="4" class="text-end border-top-2 pt-3">Total Keseluruhan:</td>
                                            <td class="text-end border-top-2 pt-3">Rp <?php echo number_format($order_details['total_price'], 0, ',', '.'); ?></td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
